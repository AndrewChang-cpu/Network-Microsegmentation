...
for opt in "${HPING_OPTIONS[@]}"; do
    for i in {1..3}; do
        ...
        kubectl exec  drone2 -- sh -c 'ffmpeg -i input.mp4 -f mpegts udp://'"$RECEIVER_IP"':'"$VIDEO_PORT"' >/dev/null 2>&1 &' &
        sleep 2
	...
	if [ $i -eq 2 ]; then
        ...
        kubectl exec  drone2 -- pkill ffmpeg
